...
Models are ranked by CV F1, computed on the training split before the test set is touched; this development selection metric is deliberately separate from the Section 1 ROC-AUC success bar, which is checked once on the holdout and never used to select. Extra Trees has the highest CV F1 (0.498) and becomes the champion we serve, explain, and monitor for drift; its test ROC-AUC (0.666) and test F1 (0.574) clear both thresholds. The ranking is statistically fragile, though: the top three tree ensembles (0.498 / 0.489 / 0.485) all sit within one CV standard deviation (about 0.044), so under a one-standard-error rule they are effectively tied. Extra Trees is promoted because it leads on both CV F1 and holdout test F1 (0.5743 versus 0.5735 for Hist. Gradient Boosting, which edges ahead only on test ROC-AUC, 0.682); because that margin is within noise, the champion is fixed by a deliberate manual promotion gate (Section 4.2), and a re-tune that re-ranked these three would be a legitimate reason to re-promote. Logistic regression lags far behind, confirming a linear boundary does not capture the signal.

To be direct about what clearing the bar means: a ROC-AUC of 0.666 and accuracy of 0.650 are real but modest, closer to "better than a coin flip" than to a deployable diagnostic, and the same ceiling holds for every model we tried. That is expected for the Kaggle Water Potability dataset, whose labels are synthetic and only weakly tied to the nine measurements, so no classifier can go much further without more informative features. The Section 1 thresholds were set precisely to separate "learned a real signal" from "guessing," and Extra Trees clears that bar without being a clinically usable test. The more transferable outcome of this project is therefore the production-readiness work in Section 4 (serving, drift monitoring, retraining triggers), not the absolute accuracy of the classifier.
